    ...
    proc.on('close', (code) => {
      ...
      for (const line of lines) {
        try {
          ...
          rawEvents.push(raw);
          const normalized = normalizeEvents('gemini', raw);
...
describe('Gemini Live E2E', () => {
  test('should spawn gemini and parse tool calls correctly', async () => {
    ...
    console.log('Sample raw events:', rawEvents.slice(0, 5).map(r => ({ type: r.type, tool_name: r.tool_name, name: r.name })));
    ...
    const quickStatus = getQuickStatus('live-agent', 'gemini', 'completed', events);
...
8. Create a new file dir4/newfile.txt with content "new file"`;
      ...
      const quickStatus = getQuickStatus('comprehensive-agent', 'gemini', 'completed', events);
      console.log('Quick status:', quickStatus);
