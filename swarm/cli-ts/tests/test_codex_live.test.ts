...
async function runCodexAgent(prompt: string, timeoutMs: number = 60000): Promise<{ events: any[], rawEvents: any[], rawStdout: string }> {
  return new Promise((resolve, reject) => {
    const args = ['exec', '--sandbox', 'workspace-write', prompt, '--full-auto', '--json'];
    ...
    proc.on('close', (code) => {
      ...
      for (const line of lines) {
        try {
          ...
          rawEvents.push(raw);
          const normalized = normalizeEvents('codex', raw);
          allEvents.push(...normalized);
        } catch (e) {
        }
      }

      if (rawEvents.length === 0 && stdout.trim()) {
        try {
...
describe('Codex Live E2E', () => {
  test('should spawn codex and parse tool calls correctly', async () => {
    ...
    const { events, rawEvents, rawStdout } = await runCodexAgent(prompt, 120000);

    const testdataDir = join(__dirname, 'testdata');
    const logFilePath = join(testdataDir, 'codex-agent-log-simple.jsonl');
    ...
    const quickStatus = getQuickStatus('live-agent', 'codex', 'completed', events);
...
8. Create a new file dir4/newfile.txt with content "new file"`;
      ...
      const { events, rawEvents, rawStdout } = await runCodexAgent(prompt, 180000);
      
      const logFilePath = join(testdataDir, 'codex-agent-log-comprehensive.jsonl');
      writeFileSync(logFilePath, rawStdout, 'utf-8');
      console.log(`Saved full stdout to: ${logFilePath}`);
      ...
      for (const raw of rawEvents) {
        ...
        if (raw.item?.type) {
          const itemType = `item.${raw.item.type}`;
          rawTypeCounts[itemType] = (rawTypeCounts[itemType] || 0) + 1;
        }
      }
      console.log('Raw event type counts:', rawTypeCounts);
      console.log('Sample raw events:', rawEvents.slice(0, 5).map(r => ({ 
        type: r.type, 
        item: r.item ? { type: r.item.type, name: r.item.name } : null 
      })));
      ...
      const quickStatus = getQuickStatus('comprehensive-agent', 'codex', 'completed', events);
      console.log('Quick status:', quickStatus);
